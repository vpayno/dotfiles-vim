...
function downloadKiteInstaller() {
	...
	if  command -v wget > /dev/null 2>&1; then
		...
		wget     -q -O "$target" "$URL" || {
			echo                                     >&2 "Failed to download $target. Run 'wget -O \"$target\" \"$URL\"' for more information. Exiting now."
			exit                                                                                                                                                   1
		}
	elif  command -v curl > /dev/null 2>&1; then
		echo     "Downloading $target binary using curl..."
		curl     -L --output "$target" "$URL" || {
			echo                                           >&2 "Failed to download $target. Run 'curl -L --output \"$target\" \"$URL\" for more information. Exiting now."
			exit                                                                                                                                                                 1
		}
	else
	...
	[  -f "$target" ] || {
		echo                       >&2 "Unable to locate downloaded file $target. terminating."
		exit                                                                                          1
	}
	chmod  u+x "$downloadFile" || {
		echo                                >&2 "Failed to make $downloadFile executable. Run 'chmod u+x $downloadFile' for more information. Exiting now."
		exit                                                                                                                                                      1
	}
}
...
function downloadKitePackage() {
	...
	[  -x "$target" ] || {
		echo                       >&2 "Unable to locate executable file $target. terminating."
		exit                                                                                          1
	}

	"$target"  install --download || {
		echo                                   >&2 "Unable to download Kite executable package. terminating."
		exit                                                                                                        1
	}
}
...
function installKite() {
	...
	[  -f "$downloadFile" ] || {
		echo                             "Unable to locate kite-installer at $downloadFile. Exiting now."
		exit                                                                                                    1
	}
